...
from src.models import ModelFactoryRegistry, UNet, UNetImproved


class ConfigFalso:
    """Imita o TrainConfig, que é o que a factory consome."""
...
def test_diferenca_esta_so_nas_cabecas():
    ...
    cabeca_p1 = sum(p.numel() for p in p1.head.parameters())

    assert diferenca_total == cabecas_p2 - cabeca_p1
    # a diferença é irrelevante perto do total (menos de 0,1%)
    assert diferenca_total / sum(p.numel() for p in p1.parameters()) < 0.001
...
def test_treino_de_ponta_a_ponta():
    ...
    alvos = modelo.build_targets(labels, torch.device("cpu"))
    perda, componentes = modelo.compute_loss(modelo(torch.randn(1, 1, 32, 32)), alvos)
    perda.backward()
...
def test_factory_escolhe_pelo_nome():
    """O quê: `model.name` no config seleciona a arquitetura.
    Por quê: é o que permite rodar as Partes 1 e 2 com o mesmo comando, trocando só o YAML.
    Como: constrói os dois pelo registry.
    """
    base = {"base": 8, "depth": 2}
    assert isinstance(ModelFactoryRegistry.build(ConfigFalso({"name": "unet", **base})), UNet)
    assert isinstance(
        ModelFactoryRegistry.build(ConfigFalso({"name": "unet_improved", **base})), UNetImproved
    )


def test_factory_assume_unet_sem_nome():
    """O quê: config sem `model.name` constrói a UNet binária.
    ...
    """
    modelo = ModelFactoryRegistry.build(ConfigFalso({"base": 8, "depth": 2, "out_channels": 1}))
    assert isinstance(modelo, UNet)
    assert not isinstance(modelo, UNetImproved)
...
def test_factory_repassa_config_da_perda():
    """O quê: os pesos do YAML chegam na perda do modelo.
    ...
    cfg = ConfigFalso({"name": "unet_improved", "base": 8, "depth": 2,
                       "loss": {"w_offset": 0.25, "pos_weight": 42.0}})
    modelo = ModelFactoryRegistry.build(cfg)

    assert modelo.criterion.w_offset == 0.25
    assert modelo.criterion.pos_weight == 42.0
